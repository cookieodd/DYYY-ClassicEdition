...
- (NSArray *)dataArray {
    ...
    BOOL hideBGAudioSettings = [[NSUserDefaults standardUserDefaults] boolForKey:@"DYYYHidePanelBGAudioSettings"];
            ...
            for (id item in groupModel.groupArr) {
                if ([item isKindOfClass:%c(AWELongPressPanelBaseViewModel)]) {
                    ...
                    if (([descString isEqualToString:@"转发到日常"] || [descString isEqualToString:@"分享到日常"]) && hideDaily) {
                        ...
                        shouldHide = YES;
                    } else if ([descString isEqualToString:@"后台音频设置"] && hideBGAudioSettings) {
                        shouldHide = YES;
