...
@property(nonatomic, strong) NSArray<NSArray<DYYYSettingItem *> *> *filteredSettingSections;
...
- (void)setupSearchBar {
    ...
    if (@available(iOS 13.0, *)) {
        self.searchBar.searchTextField.backgroundColor = [UIColor colorWithWhite:0.2 alpha:0.3];
        ...
        self.searchBar.searchTextField.inputAccessoryView = toolbar;
    }
}
...
- (void)setupTableView {
    ...
    // 搜索栏
    UIView *headerContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 60)];
    headerContainer.backgroundColor = [UIColor clearColor];
    self.searchBar.frame = CGRectMake(8, 5, self.view.bounds.size.width - 16, 50);
...
- (void)setupSettingItems {
    self.settingSections = @[
        ...
        @[
            ...
            [DYYYSettingItem itemWithTitle:@"隐藏后台音频设置" key:@"DYYYHidePanelBGAudioSettings" type:DYYYSettingItemTypeSwitch],
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    return self.isSearching ? self.filteredSettingSections.count : self.settingSections.count;
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (self.isSearching) {
        // 搜索时展开所有
        return self.filteredSettingSections[section].count;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    // 根据搜索状态选择数据源
    NSArray *sections = self.isSearching ? self.filteredSettingSections : self.settingSections;
...
- (void)filterContentForSearchText:(NSString *)searchText {
    if (!searchText || searchText.length == 0) {
        self.filteredSettingSections = nil;
    ...
    for (NSInteger i = 0; i < self.settingSections.count; i++) {
        ...
        if (filteredSection.count > 0) {
            [filtered addObject:filteredSection];
            [filteredTitles addObject:self.sectionTitles[i]];
        }
    }
    
    self.filteredSettingSections = filtered;
